      ...
      <div className="w-full max-w-md">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="overflow-hidden rounded-lg border border-slate-800 bg-slate-900/80 shadow-2xl shadow-black/40">
          ...
          <div className="border-t border-slate-800 px-7 py-4 text-center text-xs text-slate-500">
            ...
            {mode === 'verify' && <><button type="button" onClick={async () => { await fetch(`${API_BASE}/auth/resend-verification`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: verificationEmail }) }); setMessage({ text: 'A new verification code was sent.', error: false }); }} className="text-cyan-400 hover:underline">Resend code</button><button type="button" onClick={() => setMode('login')} className="ml-3 text-cyan-400 hover:underline">Back to sign in</button></>}
