      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_0.8fr]">
        <article className="rounded-lg border border-slate-800 bg-slate-900/80 p-6">
          <div className="mb-4 flex flex-wrap gap-3">
            ...
            <button onClick={enqueue} className="rounded-lg bg-cyan-400 px-4 py-2 text-sm font-semibold text-slate-950">Enqueue</button>
            <button onClick={dequeue} className="rounded-lg border border-slate-700 px-4 py-2 text-sm font-semibold text-slate-100">Dequeue</button>
            <button onClick={front} className="rounded-lg border border-slate-700 px-4 py-2 text-sm font-semibold text-slate-100">Front</button>
            <button onClick={rear} className="rounded-lg border border-slate-700 px-4 py-2 text-sm font-semibold text-slate-100">Rear</button>
          </div>
          <div className="rounded-lg border border-slate-800 bg-slate-950 p-4">
            <div className="flex flex-wrap items-center gap-3 rounded-lg border border-cyan-400/30 bg-slate-900 p-4">
              {queue.map((item, index) => (
                <div key={`${item}-${index}`} className="rounded-lg border border-cyan-400/30 bg-cyan-400/10 px-4 py-3 text-sm font-semibold text-slate-100">{item}</div>
              ))}
        ...
        <aside className="rounded-lg border border-slate-800 bg-slate-900/80 p-6 text-slate-200">
          ...
          <div className="mt-4 space-y-3 text-sm text-slate-300">
            <div className="rounded-lg border border-slate-800 bg-slate-950/80 p-4">FIFO: The first item inserted is the first item removed.</div>
            <div className="rounded-lg border border-slate-800 bg-slate-950/80 p-4">Front and rear pointers are updated after each operation.</div>
