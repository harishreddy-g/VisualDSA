      ...
      <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <article className="rounded-lg border border-slate-800 bg-slate-900/80 p-6">
          <div className="mb-4 flex flex-wrap gap-3">
            <input value={frontValue} onChange={(e) => setFrontValue(e.target.value)} placeholder="Front value" className="rounded-lg border border-slate-700 bg-slate-950 px-4 py-2 text-sm text-slate-100" />
            <button onClick={insertFront} className="rounded-lg bg-cyan-400 px-4 py-2 text-sm font-semibold text-slate-950">Insert Front</button>
            <input value={endValue} onChange={(e) => setEndValue(e.target.value)} placeholder="End value" className="rounded-lg border border-slate-700 bg-slate-950 px-4 py-2 text-sm text-slate-100" />
            <button onClick={insertEnd} className="rounded-lg border border-slate-700 px-4 py-2 text-sm font-semibold text-slate-100">Insert End</button>
          </div>
          <div className="mb-4 flex flex-wrap gap-3">
            <input value={target} onChange={(e) => setTarget(e.target.value)} placeholder="Delete / Search value" className="rounded-lg border border-slate-700 bg-slate-950 px-4 py-2 text-sm text-slate-100" />
            <button onClick={deleteNode} className="rounded-lg border border-slate-700 px-4 py-2 text-sm font-semibold text-slate-100">Delete Node</button>
            <button onClick={searchNode} className="rounded-lg border border-slate-700 px-4 py-2 text-sm font-semibold text-slate-100">Search Node</button>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 rounded-lg border border-slate-800 bg-slate-950 p-4">
            {nodes.map((value, index) => (
              <div key={`${value}-${index}`} className="flex items-center gap-2">
                <div className={`rounded-lg border px-4 py-3 text-sm font-semibold text-slate-100 ${active === value ? 'border-amber-300 bg-amber-400/20' : 'border-cyan-400/30 bg-cyan-400/10'}`}>
                  {value}
                </div>
        ...
        <aside className="space-y-4 rounded-lg border border-slate-800 bg-slate-900/80 p-6 text-slate-200">
          ...
          <div className="rounded-lg border border-slate-800 bg-slate-950/80 p-4">Node Count: {summary.count}</div>
          <div className="rounded-lg border border-slate-800 bg-slate-950/80 p-4">Head: {summary.head}</div>
          <div className="rounded-lg border border-slate-800 bg-slate-950/80 p-4">Tail: {summary.tail}</div>
          <div className="rounded-lg border border-slate-800 bg-slate-950/80 p-4 text-sm text-slate-300">Operations update the list in real time and highlight the active node being inserted, deleted, or searched.</div>
